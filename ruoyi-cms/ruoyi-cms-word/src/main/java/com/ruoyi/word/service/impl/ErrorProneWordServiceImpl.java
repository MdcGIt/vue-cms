...
public class ErrorProneWordServiceImpl extends ServiceImpl<CmsErrorProneWordMapper, CmsErrorProneWord>
	...
	public Map<String, String> getErrorProneWords() {
		Map<String, String> cacheMap = this.redisCache.getCacheMap(CACHE_KEY, () -> this.list().stream()
				.collect(Collectors.toMap(CmsErrorProneWord::getWord, CmsErrorProneWord::getReplaceWord)));
		return cacheMap;
	}

	@Override
	public void addErrorProneWord(CmsErrorProneWord word) {
		Long count = this.lambdaQuery().eq(CmsErrorProneWord::getWord, word.getWord()).count();
	...
	@Override
	public void updateErrorProneWord(CmsErrorProneWord word) {
		CmsErrorProneWord db = this.getById(word.getWordId());
		Assert.notNull(db, () -> CommonErrorCode.DATA_NOT_FOUND_BY_ID.exception("wordId", word.getWordId()));

		Long count = this.lambdaQuery().eq(CmsErrorProneWord::getWord, word.getWord())
				.ne(CmsErrorProneWord::getWordId, word.getWordId()).count();
		Assert.isTrue(count == 0, () -> WordErrorCode.CONFLIECT_ERROR_PRONE_WORD.exception(word.getWord()));
