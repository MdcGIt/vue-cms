...
public class ChineseSpelling {
	 ...
	 */
	public static int getGBKCode(char cn) {
		byte[] bytes = null;
		try {
	 ...
	 */
	private static String getSpellByGBKCode(int code) {
		...
		int asciiRang;
		for (Entry<String, Integer> e : spellMap.entrySet()) {
			asciiRang = e.getValue();
			if (code >= asciiRang0 && code < asciiRang) { // 区间找到
				return spell0 == null ? e.getKey() : spell0;
			} else {
				spell0 = e.getKey();
	...
	public static String getSpelling(String str) {
		...
		for (char c : chars) {
			...
			if (code == 0) { // 取ascii时出错
				...
				String spell = getSpellByGBKCode(code);
				if (spell == null) {
					sb.append(convert2(c));
				} else {
					sb.append(spell);
				}
			}
	 ...
	 * 得到姓名第一个汉字的全拼
	 */
	public static String getPersonNameSpelling(String str) {
		if (str == null) {
			return null;
		}
		String first = str.substring(0, 1);
		String v = specialFamilyNames.get(first);
		if (v != null) {
			return v;
		}
		return getSpelling(first);
	}

	/**
	 * 返回由字符串中每个汉字拼音首字母组成的字符串，非汉字字符维持原样输出
	 * 
	 * @param SourceStr
	 * @return
	 */
	public static String getCapitalizedSpelling(String SourceStr) {
	...
	static {
		specialFamilyNames.put("翟", "Zhai");
		specialFamilyNames.put("朴", "Piao");
		specialFamilyNames.put("缪", "Miao");
		specialFamilyNames.put("卜", "Bu");
		specialFamilyNames.put("睢", "Sui");
		specialFamilyNames.put("尉", "Yu");
		specialFamilyNames.put("臧", "Zang");
		specialFamilyNames.put("薄", "Bo");
		specialFamilyNames.put("瞿", "Qu");
		specialFamilyNames.put("曾", "Zeng");
		specialFamilyNames.put("潘", "Pan");
		specialFamilyNames.put("单", "Shan");
		specialFamilyNames.put("蒿", "Hao");
		specialFamilyNames.put("逯", "Lu");

		specialFamilyNames.put("解", "Xie");
		specialFamilyNames.put("查", "Zha");
		specialFamilyNames.put("於", "Yu");
		specialFamilyNames.put("隗", "Kui");
		specialFamilyNames.put("折", "She");
		specialFamilyNames.put("阙", "Que");
		specialFamilyNames.put("区", "Ou");
		specialFamilyNames.put("仇", "Qiu");
		specialFamilyNames.put("颉", "Xie");

		specialFamilyNames.put("盖", "Ge");
		specialFamilyNames.put("逄", "Pang");
		specialFamilyNames.put("汶", "Wen");
		specialFamilyNames.put("乐", "Yue");
		specialFamilyNames.put("阚", "Kan");
		specialFamilyNames.put("眭", "Sui");
		specialFamilyNames.put("澹", "Tan");

		specialFamilyNames.put("宓", "Mi");
		specialFamilyNames.put("蔚", "Yu");
		specialFamilyNames.put("郗", "Xi");
		specialFamilyNames.put("种", "Chong");
		specialFamilyNames.put("柏", "Bo");
		specialFamilyNames.put("乜", "Nie");
		specialFamilyNames.put("伽", "Qie");
		specialFamilyNames.put("藏", "Zang");

		specialFamilyNames.put("朝", "Zhao");
		specialFamilyNames.put("缐", "Xian");
		specialFamilyNames.put("佴", "Nai");
		specialFamilyNames.put("禇", "Chu");
		specialFamilyNames.put("澹", "Tan");
	}

	private static String[][] all = new String[][] {
			new String[] { "A", "啊阿呵吖嗄腌锕錒" },
			new String[] { "Ai", "爱矮挨哎碍癌艾唉哀蔼隘埃皑呆嗌嫒瑷暧捱砹嗳锿霭乂乃伌僾儗凒剀剴叆呃呝啀嘊噫噯堨塧壒奇娭娾嬡嵦愛懓懝敱敳昹曖欬欸毐溰溾濭烠焥璦皚皧瞹硋磑礙絠薆藹諰譪譺賹躷醷鎄鑀閡阂阨阸隑靄靉餲馤騃鯦鱫鴱" },
			...
			new String[] { "Beng", "蹦绷甭崩迸蚌泵甏嘣伻傍傰唪嗙埄埲堋塴奟嵭抨挷搒榜漨熢琣琫痭祊絣綳繃菶跰逬錋鏰镚閍鞛" },
			new String[] {
					"Bi",
			...
			new String[] { "Chen", "趁称辰臣尘晨沉陈衬橙忱郴榇抻碜谶宸龀嗔伧琛侲傖儭嚫堪塡填塵墋夦帘愖捵揨敐曟枕桭棧棽樄櫬沈谌湛瀋烥煁疢疹瘎瘨眈瞋硶磣稱綝縝缜肜胂茞莀莐蔯薼螴襯訦諃諶謓謲讖賝贂趂趻跈踸軙迧醦鈂鍖闖闯陳霃鷐麎齓齔秤" },
			new String[] { "Cheng",
			...
			new String[] { "Du", "读度毒渡堵独肚镀赌睹杜督都犊妒顿蠹笃嘟渎椟牍黩髑芏儥凟剢剫匵厾噣土塗妬嬻宅帾斁晵暏樚樞橐櫝殬殰涜瀆牘犢獨琽瓄皾睪秺竇笁篤纛荰蝳螙蠧裻襡襩覩詫読讀讟诧豄賭贕醏錖鍍鍺鑟锗闍阇陼靯韇韣韥頓騳黷" },
			...
			new String[] {
					"Fu",
					"副幅扶浮富福负伏付复服附俯斧赴缚拂夫父符孵敷赋辅府腐腹妇抚覆辐肤氟佛俘傅讣弗涪袱市甫釜脯腑阜咐黼砩苻趺跗蚨芾鲋幞茯滏蜉拊菔蝠鳆蝮绂绋赙罘稃匐麸凫桴莩孚馥驸怫祓呋郛芙艴黻不乀仅伕俌俛偩偪冨冹刜包呒咈哹哺嘸坿垘垺報妋姇娐婏婦媍嬎嬔尃岪峊巿帗弣彳彿復怀怤懯抙捊捬掊撫旉枎枹柎柫柭栿棴椨椱榑汱沕沸泭洑溥澓炥烰焤玞玸琈璷甶畉畐畗癁盙砆祔禣秿稪竎笰筟箁箙簠粰糐紨紱紼絥綍綒緮縛纀罦翇胕膚艀芣茀荂荴莆萉萯葍蓲蕧虙蚥蚹蛗蜅蝜衭袚袝複褔襆襥覄訃詂諨豧負費賦賻费踾軵輔輹輻还邚邞郍郙鄜酜酻釡鈇鉘鉜錇鍑鍢锫阝陚鞴韍韛韨頫颫颰駙髴鬴鮄鮒鮲鰒鳧鳬鳺鴔鵩鶝麩麬麱" },
			...
			new String[] { "Gang", "刚钢纲港缸岗杠冈肛扛筻罡戆剛堈堽岡崗戅戇抗掆棡槓溝焵牨犅犺疘矼碙綱罁罓肮釭鋼鎠阬頏颃" },
			new String[] { "Gao", "高搞告稿膏篙羔糕镐皋郜诰杲缟睾槔锆槁藁勂吿咎夰峼暠槀槹橰檺櫜滜澔獋獔皐睪祮祰禞稁稾筶縞羙臯菒蒿藳誥鋯鎬韟餻髙鷎鷱鼛" },
			new String[] { "Ge",
					"个各歌割哥搁格阁隔革咯胳葛蛤戈鸽疙盖屹合铬硌骼颌袼塥虼圪镉仡舸鬲嗝膈搿纥哿介佫佮個匌可吤呄嘅嘢噶彁愅戓戨扢挌擱敋杚槅櫊滆滒澔牫牱犵猲獦砝秴箇紇肐臈臵茖菏蓋蛒裓觡詥諽謌輵轕鉀鉻鉿鎑鎘鎶钾铪閘閣閤闸鞈鞷韐韚頜騔髂魺鮥鮯鰪鲄鴐鴚鴿鵅" },
			new String[] { "Gei", "给" },
			...
			new String[] { "Guan", "关管官观馆惯罐灌冠贯棺纶盥矜莞掼涫鳏鹳倌丱串卝婠悹悺慣懽摜斡果桄樌櫬權毌沦泴淉淪潅爟琯瓘痯瘝癏矔礶祼窤筦綸罆舘萖蒄覌観觀貫躀輨遦錧鏆鑵閞関闗關雚館鰥鱞鱹鳤鵍鸛" },
			...
			new String[] { "Hal", "乤" },
			new String[] { "Han",
					"喊含汗寒汉旱酣韩焊涵函憨翰罕撼捍憾悍邯邗菡撖瀚阚顸蚶焓颔晗鼾仠佄傼兯凾厂厈咁哻唅嚂圅垾娢嫨屽崡嵅嵌忓感扞攼旰晘晥暵桿梒椷榦欦歛汵泔浛浫涆淊淦滩漢澉澏澣灘熯爳猂琀甘甝皔睅矸笒筨糮肣莟蔊蘫虷蛿蜬蜭螒譀谽豃軒轩釬鈐銲鋎鋡閈闞闬雗靬韓頇頜頷顄顩馠馯駻鬫魽鳱鶾" },
			...
			new String[] {
					"Ji",
					"几及急既即机鸡积记级极计挤己季寄纪系基激吉脊际汲肌嫉姬绩缉饥迹棘蓟技冀辑伎祭剂悸济籍寂期其奇忌齐妓继集给革击圾箕讥畸稽疾墼洎鲚屐齑戟鲫嵇矶稷戢虮诘笈暨笄剞叽蒺跻嵴掎跽霁唧畿荠瘠玑羁丌偈芨佶赍楫髻咭蕺觊麂骥殛岌亟犄乩芰哜丮乁亼伋倚偮僟兾刉刏剤劑勣卙卟卽厝叝吇呰喞嗘嘰嚌坖垍堲塉墍妀姞姼尐居屰岋峜嵆嶯帺幾庴廭彐彑彶徛忣惎愱憿懠懻揖揤撃撠撽擊擠攲敧旡旣暩曁枅梞棋楖極槉槣樭機橶檕檝檵櫅櫭毄汥泲洁淁済湒漃漈潗濈濟瀱焏犱狤猗璂璣璾畟疵痵瘵癠癪皀皍睽瞉瞿磯禝禨秸稘稩穄穊積穖穧筓箿簊簎粢糭紀紒級結給継緝縘績繋繫繼结罽羇羈耤耭胔脔脨膌臮艥艻芶苙茍茤莋萁萕葪蒩蔇蕀蕲薊薺藉蘄蘎蘮蘻虀蜡蝍螏蟣蟻蟿蠀裚褀襀襋覉覊覘覬覿觇觌觙觭計訐記誋諅諔譏譤讦谻谿賫賷趌趞跂跡踑踖踦蹐蹟躋躤躸輯轚郆鄿銈銡錤鍓鏶鐖鑇鑙隔際隮雞雦雧霵霽鞊鞿韲颳飢饑騎驥骑鬾魝魢魥鮆鯚鯯鯽鰶鰿鱀鱭鱾鳮鵋鶏鶺鷄鷑鸄鹡齊齌齍齎齏" },
			new String[] { "Jia",
					"家加假价架甲佳夹嘉驾嫁枷荚颊钾稼茄贾铗葭迦戛浃镓痂恝岬跏嘏伽胛笳珈瘕郏袈蛱傢價叚呷咖唊圿埉夏夓夾婽宊幏徦忦戞扴抸押拁拮挈挟挾揩揳擖斚斝暇梜椵榎榢槚檟毠泇浹犌猰猳玾筴糘耞脥腵莢蛺蝦袷裌豭貑賈跲郟鉀鉫鉿鋏鎵铪頡頬頰颉餄饸駕駱骆骱鴶鵊麚" },
			new String[] {
					"Jian",
					"见件减尖间键贱肩兼建检箭煎简剪歼监坚奸健艰荐剑渐溅涧鉴浅践捡柬笺俭碱硷拣舰槛缄茧饯翦鞯戋谏牮枧腱趼缣搛戬毽菅鲣笕谫楗囝蹇裥踺睑謇鹣蒹僭锏湔侟俴倹偂傔僣儉冿剣剱劍劎劒劔喊囏堅堑堿塹墹姦姧孱寋帴幵弿彅徤惤戔戩挸揀揃揵撿擶攕旔暕朁柙栫梘検椷椾榗樫橌橏橺檢檻櫼殱殲洊涀淺減湕滥漸澗濫濺瀐瀳瀸瀽熞熸牋犍猏玪珔瑊瑐監睷瞯瞷瞼碊磵礀礆礛稴筧箋箴篯簡籈籛糋絸緘縑繝繭纎纖纤聻臶艦艱茛菺葌葏葥蔪蕑蕳薦藆虃螹蠒袸襇襉襺見覵覸詃諓諫謭譖譼譾谮豜豣賎賤趝跈踐轞醎醶釰釼鈃銒銭鋄鋑鋻錢錬錽鍊鍳鍵鎫鏩鐗鐧鐱鑑鑒鑬鑯鑳钘钱閒間险險靬鞬韀韉餞餰馢騫鬋鰎鰔鰜鰹鳒鳽鵳鶼鹸鹹鹻鹼麉黚黬廴" },
			...
			new String[] {
					"Ju",
					"句举巨局具距锯剧居聚拘菊矩沮拒惧鞠狙驹且据柜桔俱车咀疽踞炬倨醵裾屦犋苴窭飓锔椐苣琚掬榘龃趄莒雎遽橘踽榉鞫钜讵枸仇伡佝侷俥倶僪冣凥劇勮匊告圧坥埧埾壉姐姖娵娶婅婮寠屨岠岨崌巈弆忂怇怐怚愳懅懼抅拠拱挙挶捄揈揟據擧昛梮椇椈檋櫸欅歫毩毱泃泦洰涺淗渠湨澽焗焣爠犑狊珇痀眗瞿砠租秬窶筥篓簍簴籧粔粷罝耟聥腒臄舉艍菹萭葅蒌蒟蒩蓻蔞蘜蘧處虡蚷蛆蜛螶袓襷詎諊豦貗趉趜趡跔跙跼踘蹫蹻躆躹軥輂邭邹郥郰郹鄒鄹鉅鉏鋤鋦鋸鐻锄閰陱雏雛颶駏駒駶驕驧骄鬻鮈鮍鮔鲏鴡鵙鵴鶋鶪鼰鼳齟" },
			...
			new String[] { "Sheng", "声省剩生升绳胜盛圣甥牲乘晟渑眚笙嵊丞乗偗剰勝呏垩墭姓娍媵憴斘昇晠曻枡榺橳殅殸泩渻湦澠焺狌珄琞甸竔箵縄繩聖聲苼蕂譝貹賸鉎鍟阩陞陹鱦鵿鼪" },
			new String[] {
					"Shi",
			...
			new String[] {
					"Zhi",
					"只之直知制指纸支芝枝稚吱蜘质肢脂汁炙织职痔植抵殖执值侄址滞止趾治旨窒志挚掷至致置帜识峙氏智秩帙摭黹桎枳轵忮祉蛭膣觯郅栀彘芷祗咫鸷絷踬胝骘轾痣陟踯雉埴贽卮酯豸跖栉乿亊伎俧倁値偫傂儨凪剬劕劧厔咥嚔坁坧垁埃執墆墌多夛妷姪娡媞嬂实寘實崻巵帋幟庢庤廌徏徔徝徴徵恃恉慹憄懥懫戠扺扻抧拓拞挃捗搘搱摕摨摯擲擳擿斦旘昵晊杝杫柣栺栽梔梽椥楖榰槜樀樲樴櫍櫛歭氐汥汦沚泜洔洷淽滍滯漐潌潪瀄熫犆狾猘瓆瓡畤疐疷疻瘈眰砋礩示祇祑祬禃禔禵秇秓秖秪积秲秷稙稺穉筫紙紩絺綕緻縶織翐耆聀職胑胵膱臷臸芖茋茝菭薙藢蘵虒蚔螲蟙衹衼袟袠製襧覟觗觝觶訨誌謢識豑豒貭質贄跱踶蹛蹠蹢躑躓軄軹軽輊迟迣遟遲郦酈釞鉄銍銴鋕鑕铁铚锧阤阯陁隲隻馶馽駤騭騺驇鯯鳩鳷鴙鴲鶨鷙鸠鼅夂" },
			...
			new String[] { "Zuan", "钻纂赚缵躜攥劗揝撮攒攢欑篹籫繤纉纘賺躦鑚鑽" }, new String[] { "Zui", "最嘴醉罪堆咀觜蕞厜嗺噿嫢嶉嶊嶵摧撮晬朘枠栬槯樶檇檌欈濢璻睟祽稡穝絊纗羧脧蟕辠酔酨酻鋷錊雋" },
