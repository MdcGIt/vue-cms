...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        word: [
          { required: true, message: "易错词不能为空", trigger: "blur" }
        ],
        replaceWord: [
          { required: true, message: "替换词不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete (row) {
      ...
      this.$modal.confirm('是否确认删除选中的易错词?').then(function() {
